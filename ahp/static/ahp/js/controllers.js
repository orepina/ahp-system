...
function groupsController( $scope, ajaxFactory, updateFactory  ) {
    ...
    $scope.sendEmailHierarchy = function(user, group_id) {
        ...
        $scope.text = 'Здравствуйте, '+user.name+'! Нас интересует Ваше мнение по проблеме "'+$scope.node_hash[$scope.level_nodes_list[$scope.level_order[0]][0]].name+'". Если Вы хотите принять участие в решении данной проблемы, то пройдите анкету по ссылке в течении 2-3 дней. Затем Вам будет выслана анкета для второго этапа голосования. Спасибо за Ваше участие. Если Вы не хотите учавствовать, то просто проигнорируйте это письмо.'
    };
